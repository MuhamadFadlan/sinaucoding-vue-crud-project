      ...
      export default ({
          ...
          async viewPengguna(id){
             ...
             async deletePengguna(id){
              ...
              async loadData(){
                   let res = await axios.get(url+this.$route.query.id,{
                              headers: {
                              Authorization: `Bearer ${token}`,
                               },
                   })
                        this.addPengguna.name = res.data.name
                        this.addPengguna.email = res.data.email
                        this.addPengguna.gender = res.data.gender
                        this.addPengguna.status = res.data.status
              },
    
    
           async tambahPengguna()
           {
                  let resultTambah =  await axios.put(url+this.$route.query.id,{
